...
std::string title = "Part 3: Decision Tree Learning";
...
long long DataCollectCounter = 0;
# endif
...
class IsSpriteinNeighbouringOrSameRoom : public BehaviorTreeNode {
    ...
    virtual NodeState execute(Boid &monster) override {
        for(auto r:rooms[current_monster_room].adjacent_rooms){
            if(r==current_room){
                cout<<"Sprite in same or neighbouring room of the monster"<<endl;
                return SUCCESS;
            }
        }
        return FAILURE;
...
int main()
{
    ...
    // Decision Tree for Monster
    struct DecisionTree* rootM = new DecisionTree(0);
    rootM->left = new DecisionTree(1);
    rootM->right = new DecisionTree(2);
    rootM->left->left = new DecisionTree(3);
    rootM->left->right = new DecisionTree(4);
    rootM->right->left = new DecisionTree(5);
    rootM->right->right = new DecisionTree(6);
    rootM->left->left->left = new DecisionTree(7);
    rootM->left->left->right = new DecisionTree(8);
    rootM->right->left->left = new DecisionTree(9);
    rootM->right->left->right = new DecisionTree(10);
    ...
    ofstream csv;
    csv.open("final_dataset.csv");
    csv << "Sprite Room,Monster Room,Same Room,Distance To Char,Distance To Room Center,Action Choice,Killed\n";
	#endif /* TRAINING */
    ...
    while (window.isOpen())
    {
        ...
        {    
            ...
            // Dumping to CSV
            csv<<current_room<<",";
            csv<<current_monster_room<<",";
            csv<<(bool)(current_room == current_monster_room)<<",";
            csv<<sqrt(pow(sprite.data.position.x - monster.data.position.x, 2) + pow(sprite.data.position.y - monster.data.position.y, 2))<<",";
            csv<<sqrt(pow(monster.data.position.x - rooms[current_room].room_center.x, 2) + pow(monster.data.position.y - rooms[current_room].room_center.y, 2))<<",";
            csv<<action_choice<<",";
            csv<<reset_flag<<"\n";
            #endif /* TRAINING */
        ...
        {
            if(DataCollectCounter%10000000)
            {
                reset_flag = false;
                // Dumping to CSV
                csv<<current_room<<",";
        ...
        else if(tree_node->node_id==5)
        {
            ...
            cout<<"Moving chaacter to a random point within the same room"<<endl;
        }
        ...
        // Decision Tree for the Deacision Tree Learned Monster
        DecisionTree* mTree_node = rootM;
        while(mTree_node->left!=NULL)
        {
            if(mTree_node->node_id==0)
            {
                if(calculateDistance(monster.data.position,rooms[current_monster_room].room_center)<=327.23)
                    mTree_node = mTree_node->left;
                else
                    mTree_node = mTree_node->right;
            }
            else if(mTree_node->node_id==1)
            {
                if(calculateDistance(monster.data.position,rooms[current_monster_room].room_center)<=230.57)
                    mTree_node = mTree_node->left;
                else
                    mTree_node = mTree_node->right;
            }
            else if(mTree_node->node_id==2)
            {
                if(calculateDistance(monster.data.position,rooms[current_monster_room].room_center)<=388.87)
                    mTree_node = mTree_node->left;
                else
                    mTree_node = mTree_node->right;
            }
            else if(mTree_node->node_id==3)
            {
                if(current_room!=current_monster_room)
                    mTree_node = mTree_node->left;
                else
                    mTree_node = mTree_node->right;
            }
            else if(mTree_node->node_id==5)
            {
                if(calculateDistance(monster.data.position,rooms[current_monster_room].room_center)<=384.62)
                    mTree_node = mTree_node->left;
                else
                    mTree_node = mTree_node->right;
            }
        }
        if(mTree_node->node_id==4 || mTree_node->node_id==8)
        {
            cout<<"Monster is dancing wandering between different rooms"<<endl;
            int next_room = scripted_path.back();
            scripted_path.pop_back();
            scripted_path.insert(scripted_path.begin(), next_room);
            monster_wander_point = rooms[next_room].get_wander_point();
        }
        else if(mTree_node->node_id==6 || mTree_node->node_id==9)
        {
            cout<<"Monster is wandering within the same room"<<endl;
            monster_wander_point = rooms[current_monster_room].get_wander_point();
        }
        else if(mTree_node->node_id==7)
        {
            cout<<"Monster is chasing the character"<<endl;
            monster_wander_point = sprite.data.position;
        }
        else if(mTree_node->node_id==10)
        {
            cout<<"Monster is moving to the center of the room"<<endl;
            monster_wander_point = rooms[current_monster_room].room_center;
        }
        pathFindMonster(g, monster, monster_wander_point);
